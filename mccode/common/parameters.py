...
#         log.warn('The parameters.Value class is deprecated; switch to expression.Value/Expr instead')
#         self.data_type = data_type
#         self.value = value
#
#     @property
#     def is_zero(self):
#         return (self.data_type == Value.Type.float or self.data_type == Value.Type.int) and self.value == 0
#
#     @property
#     def is_expr(self):
#         return self.data_type == Value.Type.str and any(x in self.value for x in ('+', '/', '-', '*', '^'))
#
...
#         o_is_v = isinstance(other, Value)
#         if self.is_zero or (o_is_v and other.is_zero):
#             if not o_is_v or self.data_type == other.data_type:
#                 return Value(self.data_type, 0)
#             # Punt to avoid working out type relationships
#             return Value(Value.Type.int, 0)
#         if self.data_type == Value.Type.str or (o_is_v and other.is_a(Value.Type.str)):
#             return Value(Value.Type.str, f'{self.value} * {other.value}')
#         v = other.value if o_is_v else other
#         return self._binary_op(v, lambda x: x * v, '*')
#
#     def __truediv__(self, other):
#         o_is_v = isinstance(other, Value)
#         if (o_is_v and other.is_zero) or other == 0:
...
#         o_is_v = isinstance(other, Value)
#         if self.is_zero:
#             return other if o_is_v else Value.best(other)
#         if o_is_v and other.is_zero:
#             return self
#         if self.data_type == Value.Type.str or (o_is_v and other.is_a(Value.Type.str)):
#             return Value(Value.Type.str, f'({self.value} + {other.value})')
#         v = other.value if o_is_v else other
...
class ComponentParameter:
    ...
    def new_compatible_value(self, value):
        return self.value.compatible(value, id_ok=True)

    # def old_compatible_value(self, value):
    ...
    def compatible_value(self, value):
        return self.new_compatible_value(value)
